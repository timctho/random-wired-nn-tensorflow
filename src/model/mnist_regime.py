...
class MnistRandWireNN(RandWire):
    def __init__(self, config, is_training=True):
        super(MnistRandWireNN, self).__init__(config, is_training)

        if self.graph_def is None:
            self.graph_def = [None, None, None]

        self.rand_wire_layer_0 = RandWireLayer(
            self.base_channel, self.base_channel, self.n, self.k, self.p, self.m, 1,
    ...
    def __call__(self, inputs):
        with tf.variable_scope('Conv1'):
            out = tf.layers.conv2d(inputs, self.base_channel // 2, 3, 1, 'same', kernel_regularizer=tf.nn.l2_loss)
